...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % data.targets = number of classes * number of data cases
  % model.hid_to_class = number of classes * number of hidden units
  % hid_output = number of hidden units * number of data cases
  % data.inputs = number of input * number of data cases

  ret.input_to_hid = ((model.hid_to_class' * (class_prob - data.targets) .* (hid_output .* (1-hid_output)) * data.inputs')) / size(data.targets, 2) + model.input_to_hid * wd_coefficient;
